  ...
  return (
    <div>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-3">
        <Stat label="Proposals" value={all.length} />
        <Stat label="Active projects" value={active.length} />
        <Stat label="Earnings released" value={inr(payments.data?.totalReleased)} />
      </div>
  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        ...
        <Link to="/app/projects/new" className="rounded-md bg-teal px-4 py-2 font-semibold text-paper">
          Post a project
        </Link>
      </div>
      <div className="mt-6 grid gap-4 md:grid-cols-3">
        <Stat label="Projects" value={list.length} />
        <Stat label="Active" value={list.filter((p) => p.status === 'in_progress').length} />
        <Stat label="Spend released" value={inr(payments.data?.totalReleased)} />
      </div>
